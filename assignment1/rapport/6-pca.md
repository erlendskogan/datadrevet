...
*Resultat.* Areal og produksjon er sterkt korrelert (r = 0,90). De to første komponentene forklarer 99,97 % av variansen (Tabell 7), så vi beholdt to komponenter etter kravet om minst 95 %. Kaiser-kriteriet (egenverdi > 1) gir det samme. PC1 vekter areal og produksjon og beskriver hvor stor dyrkingen er, mens PC2 domineres av avling og beskriver produktiviteten (Figur 4). PC3 er kombinasjonen produksjon − areal − avling, altså selve sammenhengen mellom målingene, og har nesten ingen varians. Den er ikke nøyaktig null fordi +1 i log10(x+1) gjør sammenhengen bare tilnærmet lineær for de minste verdiene.

*One-hot-kolonnene.* Datasettet har 325 kolonner etter oppgave 4a, så vi testet også å ta de 317 one-hot-kolonnene med som input. De lar seg ikke komprimere. Med dem trengs 213 av 320 komponenter for å nå 95 %, mot to av tre uten, fordi hver kolonne bare har varians 0,006 og kolonnene innenfor `Area` og `Item` utelukker hverandre, så det finnes lite korrelasjon å samle. PC1 og PC2 endrer seg nesten ikke, 99,8 % og 99,2 % av vekten deres ligger fortsatt på målingene, og egenvektorene er de samme som i Tabell 7. Vi kjører derfor PCA på målingene alene.

*Effekt på datasettet.* Datasettet går fra tre til to numeriske kolonner, og komponentene er ukorrelerte. Rekonstrueres målingene fra de to komponentene, er feilen 0,03 % av variansen i treningssettet og 0,06 % i testsettet, så nesten ingen informasjon går tapt, og projeksjonen holder på usette data. Gevinsten er likevel liten. Å bare fjerne avling, som nesten kan regnes ut fra de to andre målingene (R² = 0,98), gir et tap på 0,3–0,6 % og beholder kolonner som er lette å tolke. PCA bekrefter dermed på dataene det oppgave 1a slo fast ut fra definisjonen, at de tre målingene bærer to uavhengige størrelser, og støtter valget om å holde avling utenfor featuresettet. Med bare tre målinger koster komponentene likevel mer i tolkbarhet enn de sparer.
...
|---|---:|---:|---:|---:|---:|---:|
| PC1 | 1,919 | 63,95 | 63,95 | 0,677 | 0,718 | 0,162 |
| PC2 | 1,081 | 36,02 | 99,97 | −0,333 | 0,103 | 0,937 |
| PC3 | 0,001 | 0,03 | 100,00 | −0,656 | 0,689 | −0,308 |
